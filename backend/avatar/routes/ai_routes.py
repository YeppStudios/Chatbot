...
from avatar.helpers.openai_helpers import check_scripted_response
from fastapi.security import OAuth2PasswordBearer
from avatar.models.request.ai import AskAiRequest, CancelRun, ListRuns, SubmitToolResponse
from fastapi import APIRouter, Depends, Form, UploadFile, File
from fastapi.responses import JSONResponse, StreamingResponse
from avatar.services.stream_service import event_stream
from avatar.middleware.jwt import verify_access_token
...
async def ask_ai(request: AskAiRequest):
    try:
        avatar = await db['avatars'].find_one({"openaiAssistantId": request.assistant_id})
        if not avatar:
        ...
        # Construct instructions with preprompt and preprompt_variable if it exists
        base_instructions = avatar.get('preprompt', '')
        if avatar.get('preprompt_variable'):
            base_instructions = f"{base_instructions} \n{avatar.get('preprompt_variable')}"
        ...
        if avatar.get('scripts'):
            scripted_found, scripted_response = await check_scripted_response(
                request.question, 
                avatar['avatarId']
            )
            if scripted_found:
                instructions = f"You got enlightened and Dariusz would response in the following way: '{scripted_response}'. Just repeat this answer in {request.language}."
        print(avatar.get('preprompt_variable'))
        if request.stream:
